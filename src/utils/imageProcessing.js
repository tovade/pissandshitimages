...
async function shitifyImageUltra(buffer, mimetype) {
  try {
    ...
    console.log(
      `Shitified image: original ${buffer.length} -> shitty ${shittyBuffer.length} bytes`
    );
    ...
    console.log(
      `DOUBLE SHITIFIED: ${doubleShitBuffer.length} bytes - IT'S FUCKING TERRIBLE NOW`
    );
...
async function gamblingShitifyImage(buffer, mimetype) {
  ...
  if (roll < 25) {
    ...
    result = {
      ...
      resultMessage:
        "🔥💀 EXTREME NUCLEAR DESTRUCTION! Your image got NUKED! 💀🔥",
    };
  } else if (roll < 50) {
    ...
    result = {
      ...
      rollPercentage: roll.toFixed(2),
      resultMessage:
        "💩 NORMAL SHITTIFICATION! Your image got moderately fucked! 💩",
    };
  } else {
    ...
    result = {
      ...
      rollPercentage: roll.toFixed(2),
      resultMessage:
        "✨🍀 JACKPOT! Your image survived unharmed! LUCKY BASTARD! 🍀✨",
    };
  }
