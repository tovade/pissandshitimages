...
async function loadLeaderboardData(page = 1) {
    try {
        ...
        document.getElementById('leaderboard').innerHTML = '<p>Error loading leaderboard. Please try again.</p>';
    }
}

function updatePageInfo(data) {
    document.getElementById('page-info').innerHTML = 
        `<p>Showing ${data.startIndex}-${data.endIndex} of ${data.totalImages} images</p>`;
}
...
function generatePaginationHTML(page, totalPages) {
    ...
    // Previous button
    if (page > 1) {
        html += `<a href="/leaderboard?page=${page - 1}" class="prev-next">⬅️ Previous</a>`;
    } else {
    ...
    // Next button
    if (page < totalPages) {
        html += `<a href="/leaderboard?page=${page + 1}" class="prev-next">Next ➡️</a>`;
    } else {
